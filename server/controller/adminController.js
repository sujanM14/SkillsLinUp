...
export const addStudent = async (req, res) => {
  try {
    ...
    const { prn, name, department, avatar, email, batch, year } = req.body;
    // console.log('Request Body:', req.body);
    if (!prn) {
      errors.prnError = 'PRN is required';
      return res.status(400).json(errors);
    }
    const errors = { prnError: '', emailError: '', backendError: '' };
    var username=req.body.prn;
    const existingPRN = await Student.findOne({ username });
    // console.log(existingPRN);
    if (existingPRN) {
      errors.prnError = 'PRN already exists';
      return res.status(400).json(errors);
    }
  ...
  // console.log(existingStudent);
    ...
    const defaultSurveyId = await addDefaultSurvey();
    ...
    console.error(error);
    const errors = { backendError: 'Internal Server Error' };
    return res.status(500).json(errors);
  }
};
export const addDefaultSurvey = async () => {
  try {
    ...
    const defaultSurvey = await new survey(defaultSurveyData).save();

    return defaultSurvey._id; // Return the ID of the newly created survey
  } catch (error) {
    console.error("Error adding default survey:", error);
    throw error;
  }
};
...
export const addClub = async (req, res) => {
  try {
    const { clubName, isTechnical } = req.body;
    const errors = { clubError: String };

    // Check if the club with the same name already exists
    const existingClub = await Club.findOne({ clubName });

    if (existingClub) {
      errors.clubError = "Club with this name already exists";
      return res.status(400).json(errors);
    }
